...
class Sigmoid(Activation):
    ...
    def value(self, x: np.ndarray) -> np.ndarray:
        ...
        :return: value of the activation function, with shape x.shape
        '''
        sigmoid = np.vectorize(lambda x: 1 / (1 + np.exp(-1 * self.k * x)))(x)
        return sigmoid
    ...
    def derivative(self, x: np.ndarray) -> np.ndarray:
        ...
        :return: derivative of the activation function, with shape x.shape
        '''
        dSigmoiddx = np.vectorize(lambda x: self.k * self.value(x) * (1 - self.value(x)))(x)
        return dSigmoiddx
...
class ReLU(Activation):
    ...
    def value(self, x: np.ndarray) -> np.ndarray:
        ...
        :return: value of the activation function, with shape x.shape
        '''
        relu = np.vectorize(lambda x: x if x > 0 else 0)(x)
        return relu
    ...
    def derivative(self, x: np.ndarray) -> np.ndarray:
        ...
        :return: derivative of the activation function, with shape x.shape
        '''
        dReludx = np.vectorize(lambda x: 1 if x > 0 else 0)(x)
        return dReludx
